    ...
    def __init__(self, base_dir=None, split='train', transform=None, fold="fold1", sup_type="label"):
        ...
        if self.split == 'train':
            self.all_slices = os.listdir(
                self._base_dir + "/MSCMR_train_slices")
            ...
            self.all_volumes = os.listdir(
                self._base_dir + "/MSCMR_val_volumes")
    ...
    def __getitem__(self, idx):
        ...
        if self.split == "train":
            h5f = h5py.File(self._base_dir +
                            "/MSCMR_train_slices/{}".format(case), 'r')
            image = h5f['image'][:]
            label = h5f['scribble'][:]
            sample = {'image': image, 'label': label}
        else:
            h5f = h5py.File(self._base_dir +
                            "/MSCMR_val_volumes/{}".format(case), 'r')
            image = h5f['image'][:]
            label = h5f['label'][:]
            sample = {'image': image, 'label': label}
        if self.split == "train":
