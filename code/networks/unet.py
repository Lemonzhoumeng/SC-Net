...
class UNet_CCT(nn.Module):
    ...
    def forward(self, x):
        feature = self.encoder(x)
        main_seg,feature1 = self.main_decoder(feature)
        aux1_feature = [Dropout(i) for i in feature]
        aux_seg1,feature_aux = self.aux_decoder1(aux1_feature)
        return main_seg,feature1, aux_seg1, feature_aux

class UNet_CCTZM(nn.Module):
    def __init__(self, in_chns, class_num):
        super(UNet_CCTZM, self).__init__()
