...
# -*- coding: utf-8 -*-
"""
Created on Mon Jan  2 22:00:59 2023
...
def train(args, snapshot_path):
    ...
    db_train = build(image_set='train', args=args)
    print('Building validation dataset...')
    db_val = build(image_set='val', args=args)
    trainloader = DataLoader(db_train, batch_size=args.batch_size, shuffle=True,
                             num_workers=8, pin_memory=True, worker_init_fn=worker_init_fn)
    valloader = DataLoader(db_val, batch_size=1, shuffle=False,num_workers=1)
    print('Number of training images: {}'.format(len(trainloader)))
    print('Number of val images: {}'.format(len(valloader)))
    model.train()
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            if iter_num > 0 and iter_num % 20 == 0:
                ...
                for i_batch, sampled_batch in enumerate(valloader):
                    ...
                    label_batch = label_all_batch["masks"]
                    volume_batch, label_batch = volume_batch.cuda(), label_batch.cuda() 
                    metric_i,label,prediction = test_single_volume_cct(volume_batch, label_batch, model, classes=num_classes)
                ...
                logging.info(
                    'iteration %d : mean_dice : %f mean_hd95 : %f' % (iter_num, performance, performance))
                model.train()
