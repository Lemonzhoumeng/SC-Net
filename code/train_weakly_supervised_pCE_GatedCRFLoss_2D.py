...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            loss_noise_robust = torch.mean(loss_vector)    
            ...
            loss = loss_ce + 0.1 * out_gatedcrf + 0.1 * loss_noise_robust + 0.1 * ent_loss
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            ...
            writer.add_scalar('info/loss_ce_super', loss_ce, iter_num)
            logging.info(
                'iteration %d : loss : %f, loss_ce: %f, loss_ce_super: %f, out_gatedcrf: %f ' %
                (iter_num, loss.item(), loss_ce.item(), loss_ce_super.item(),out_gatedcrf.item()))
