...
class NoiseRobustDiceLoss(nn.Module):
    ...
    def forward(self, predict, soft_y):
        ...
        soft_y = self._one_hot_encoder(soft_y,4)
...
class VAT2d(nn.Module):
    ...
    def forward(self, model, x):
        with torch.no_grad():
            ...
            pred_aux = F.softmax(model(x)[2], dim=1)
        ...
        with _disable_tracking_bn_stats(model):
            ...
            for _ in range(self.ip):
                ...
                pred_hat_aux = model(x + self.xi * d)[2]
                logp_hat_aux = F.softmax(pred_hat_aux, dim=1)
                adv_distance = self.loss(logp_hat, pred)
                adv_distance_aux = self.loss(logp_hat_aux, pred_aux)
                adv_distance = adv_distance + adv_distance_aux
            ...
            lds2 = self.loss(logp_hat_aux, pred_aux)
            lds = lds + lds2
        return lds
